...
model_base_name = "./opt_summarise_models/checkpoint-"
...
np.save("bleu_scores_summ_opt_final.npy", structured_array)

epochs = list(rg_scores.keys())
scores = np.array(list(rg_scores.values()))
structured_array = np.column_stack((epochs, scores))
np.save("rouge1_scores_summ_opt_final.npy", structured_array)