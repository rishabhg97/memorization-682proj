...
model_base_name = "./opt_qa_models/checkpoint-"
# tokenizer_base_name='./models/gpt2tok_cnn_dailymail_epoch'
model_names=[487,974,1461,1948,2435,2922]

bleu_scores={}
rg_scores={}
for epoch in range(0, epochs):
    model_name = f"{model_base_name}{str(model_names[epoch])}"
    ...
    with torch.no_grad():
      for i, batch in enumerate(tqdm(evaldata_loader,desc="Memorization Loop")):
          ...
          generated_sequences = model.generate(
              ...
              num_beams=5,  # Adjust the number of beams as needed
...
np.save("bleu_scores_qa_opt_beam.npy", structured_array)

epochs = list(rg_scores.keys())
scores = np.array(list(rg_scores.values()))
structured_array = np.column_stack((epochs, scores))
np.save("rouge1_scores_qa_opt_beam.npy", structured_array)