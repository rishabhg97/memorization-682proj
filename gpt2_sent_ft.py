...
def preprocess_function(examples):
  ...
  labels = ['good' if doc==1 else 'bad' for doc in examples['label']]
...
tokenized_val_dataset=raw_dataset['test'].map(
    preprocess_function,
    batched=True,
...
training_args = TrainingArguments(
    output_dir="./gpt2_sentiment_models",          # output directory
    num_train_epochs=6,              # total # of training epochs
    per_device_train_batch_size=8,  # batch size per device during training
    per_device_eval_batch_size=8,   # batch size for evaluation
    # warmup_steps=200,                # number of warmup steps for learning rate scheduler
    # weight_decay=0.01,               # strength of weight decay
    logging_dir="./gpt2_sent_logs",            # directory for storing logs
...
class SaveModelCallback(TrainerCallback):
    def on_epoch_end(self, args, state, control, **kwargs):
        output_dir = f"./gpt2-finetuned-sentiment/epoch_{state.epoch}"
